...
from transformers import EarlyStoppingCallback
from src.LP import dep_level_1, dep_level_2, dep_level_3, dep_level_4, dep_level_5, dep_level_6, job_title
...
class FunctionModel:
    ...
    def _process_row(self, row, train=False):
        if train:
            ...
            deps = [dep_level_1, dep_level_2, dep_level_3, dep_level_4, dep_level_5, dep_level_6]
            res = row['Сектор']
            job = row[job_title]
        for dep in deps:
            val = row.get(dep)
    ...
    def train(self, df):
        ...
        train_tok = train_ds.map(self._preprocess_for_train, batched=True, remove_columns=train_ds.column_names)
        val_tok = val_ds.map(self._preprocess_for_train, batched=True, remove_columns=val_ds.column_names)
        ...
        data_collator = DataCollatorForSeq2Seq(self.tokenizer, model=self.model,label_pad_token_id=-100)
    ...
    def predict(self,df, batch_size=64, num_beams=4):
        ...
        for i in tqdm(range(0, len(inputs), batch_size)):
            ...
            encoded = self.tokenizer(
                batch, return_tensors="pt", max_length=128, truncation=True, padding=True
            ).to(device)
